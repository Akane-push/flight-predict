...
async def verify_airflow_token(
    ...
    try:
        response = await http_client.get(
            "/api/v2/dags?limit=1", headers={"Authorization": f"Bearer {token}"}
        )
...
# Trigger flight dag  ================================================
@app.post(
    "/trigger-ad_hoc-flight/{date_val}/{time_val}", status_code=status.HTTP_201_CREATED
)
@limiter.limit("10/hour")
async def trigger_dag_flight(
    date_val: str,
    time_val: str,
    c_token: Annotated[str, Depends(verify_airflow_token)],
    request: Request,
):
    """Triggers the DAG
    ...
    dag_run_data = {
        ...
        "conf": {"date_val": date_val, "time_val": time_val},
    }

    try:
        response = await http_client.post(
            url, json=dag_run_data, headers={"Authorization": f"Bearer {c_token}"}
        )
...
async def trigger_dag_weather(
    ...
    dag_run_data = {"logical_date": now_utc, "conf": {"date_val": date_val}}
